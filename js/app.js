...
const state = {
  ...
  address: "TC 30/104 (1) Kamaleswaram, Manacaud PO",
...
async function renderFront() {
  ...
  // Logo (mark) top-right
  await drawLogo(c, state.logoMark, CANVAS_W - BLEED - 120, BLEED+20, 100, 100);

  // Brand name
  c.fillStyle = "#fff";
  c.font = "italic 700 50px Poppins, sans-serif";
  c.textBaseline = "top";
  c.fillText(state.brandTag.toUpperCase(), BLEED+40, BLEED+300);

  // Tagline
  c.font = "600 33px Poppins, sans-serif";
  c.fillStyle = hexToRGBA("#ffffff",0.9);
  c.fillText(state.brandName.toUpperCase(), BLEED+40, BLEED+69);
  ...
  c.fillStyle = "#fff";
  c.font = "200 26px Inter, sans-serif";
  c.fillText(state.memberName.toUpperCase() || "Member Name", BLEED+48, badgeY+33);

  // Watermark big logo-full faint
  try{
    const img = await loadImage(state.logoFull);
    const w=250, h= (img.height/img.width)*w;
    c.globalAlpha = 0.5;
    c.drawImage(img, CANVAS_W - w - 80, CANVAS_H - h - 85, w, h);
    c.globalAlpha = 1;
  }catch(e){}
...
async function renderBack() {
  ...
  // Info panel
  c.fillStyle = hexToRGBA("#000000",0.90);
  c.roundRect(BLEED, CANVAS_H-150-BLEED, CANVAS_W-BLEED*2, 150, 14); c.fill();

  // Left legal text
  c.fillStyle = "#fff";
  c.font = "600 20px Inter, sans-serif";
  c.fillText("Making You Happy", BLEED+69, BLEED+96);

  c.font = "400 18px Inter, sans-serif";
  const legalLines = (state.legal||"").split("\n");
  let ly = BLEED+300;
  legalLines.forEach(line=>{
    wrapText(c, line, BLEED+24, ly, 700, 22);
    ly += 26;
  });

  // Barcode + ID bottom-left
  const id = (state.memberId && state.memberId.trim()) || autoIdPreview();
  await drawBarcodeTo(c, id, BLEED+24, CANVAS_H-160, 420, 68, "#fff");
  c.fillStyle = "#fff"; c.font="500 20px Inter, sans-serif";
  c.fillText(id, BLEED+30, CANVAS_H-60);

  // Right contact block
  const bx = CANVAS_W - BLEED - 420;
  c.fillStyle = hexToRGBA("#000000",0.28);
  c.roundRect(bx, BLEED+0, 420, 170, 14); c.fill();

  // brand mark small
  await drawLogo(c, state.logoMark, bx+20, BLEED+20, 60, 60);

  c.fillStyle="#fff";
  c.font="700 20px Poppins, sans-serif";
  c.fillText(titleCase(state.brandName), bx+95, BLEED+40);
  c.font="500 18px Inter, sans-serif";
  c.fillText(`Ph: ${state.phone||""}`, bx+95, BLEED+70);
  c.fillText(`${state.website||""}`, bx+20, BLEED+109);
